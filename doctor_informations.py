...
def main():
    filename = 'F:/haodf-prediction/2020/2020doctor_info/2020url_60001-70000.xls'

    # 打开文件
    workBook = xlrd.open_workbook(filename)

    ## 按sheet名字获取sheet内容
    sheet_content = workBook.sheet_by_name('好大夫医生主页链接 ')

    # 3. sheet的名称，行数，列数
    print(sheet_content.name, sheet_content.nrows, sheet_content.ncols)


    url_list = sheet_content.col_values(0)
    doctor_url_list = sheet_content.col_values(1)
    del (doctor_url_list[0])

    # print("doctor_url_list:", doctor_url_list)
    ...
    for input_url in doctor_url_list:
        ...
        # 9 综合推荐热度
        print("soup.select('ul.profile-statistic > li > span')", soup.select('ul.profile-statistic > li > span'))
        ...
        for i in range(len(soup.select('section.container > header > a'))):
            if soup.select('section.container > header > a')[i].text.strip() == '查看全部问诊' not in soup.select('section.container > header > a')[i].text.strip()\
                    :
                num_inquiry = ''
            else:
                num_inquiry = soup.select('main.main-container > section > header > h2 > span > span')[0].text.strip()
        ...
        # 13 患者投票
        # print("soup.select('section.container > header > a'):", soup.select('section.container > header > a'))
        global Votes
        for i in range(len(soup.select('section.container > header > a'))):
        ...
        # 14 总文章
        print("soup.select('main.main-container > section > header > h2 > span > span')", soup.select('main.main-container > section > header > h2 > span > span'))
        global articleCount
        for i in range(len(soup.select('section.container > header > a'))):
            if soup.select('section.container > header > a')[i].text.strip() == '查看全部文章' not in soup.select('section.container > header > a')[i].text.strip()\
                    or len(soup.select('main.main-container > section > header > h2 > span > span'))== 1:
                articleCount = ''
        ...
        print("soup.select('div.aside-container > aside > div > div')", soup.select('div.aside-container > aside > div > div'))
        if soup.select('div.aside-container > aside > div > div') == [] or soup.select('div.aside-container > aside > div > div > span') == []:
            gifts = ''
        else:
    ...
    # 3.1 保存数据到Excel
    savepath = 'F:/haodf-prediction/2020/2020doctor_info/2020url_60001-70000好大夫医生主页信息.xls'
...
def get_session():
    ...
    # s.headers['Proxy-Authorization'] ='auth'
    ...
    # port = "80"
    # ip_port = ip + ":" + port
    # s.proxies = {"http": "http://" + ip_port, "https": "https://" + ip_port}
...
def saveData(lines, doctorlist, savepath):
    ...
    # 创建一个列的元组
    col = ('2医生主页链接', '3医生姓名', '4医生职称', '5教授', '6所在医院', '7科室',
